...
* The actual validation process is implemented as a [python script](https://github.com/VirtualFlyBrain/VFB_neo4j/blob/kbold2new/src/uk/ac/ebi/vfb/neo4j/neo4j_kb_validateschema.py). Its not complete in terms of validation, but its doing a few things, like checking that every node has at least one owl base type, and iri and so on. Each test is a single function in the script, so it should be fairly easy to read them over.
...
* The [process](https://github.com/VirtualFlyBrain/vfb-pipeline-dumps/blob/master/process.sh) performs the following steps (all encoded in the [Makefile](https://github.com/VirtualFlyBrain/vfb-pipeline-dumps/blob/master/dumps.Makefile)):
  1. Build dump for owlery (all logical axioms in triplestore)
  1. Build dump for pdb (VFB production instance)
